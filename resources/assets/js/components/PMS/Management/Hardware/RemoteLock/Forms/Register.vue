    ...
    <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">{{ $t('management.remotelock.form.registerTitle') }}</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form id="register-device-form" @submit.prevent="registerDevice">
            ...
            <div class="modal-footer text-right">
                <button type="submit" form="register-device-form" class="btn btn-primary" :disabled="isLoading">
                    <i v-if="isLoading" class="fa fa-circle-o-notch fa-spin mr-1" aria-hidden="true" />
                    {{ $t(isLoading ? 'management.remotelock.form.registering' : 'management.remotelock.form.register') }}
                </button>
            </div>
        </div>
    </div>
...
export default {
    ...
    methods: {
        /**
         * @name registerDevice
         * @description Register the device to a room
         *
         * @returns {void}
         */
        registerDevice() {
            ...
            axios
                .put(`remotelock_devices/${this.device.DEVICE_ID}/register`, this.form)
                .then(response => {
                    this.$emit('close')
                    this.$swal({
                        type: 'success',
                        title: this.$t('management.remotelock.form.registerSuccess'),
                        timer: 3000,
                ...
                .catch(e => {
                    if (e.response.status === 422) {
                        ...
                        this.$swal({
                            type: 'error',
                            title: this.$t('management.remotelock.form.registerFailed'),
                            timer: 3000,
                            showConfirmButton: false,
                        })
                    }
